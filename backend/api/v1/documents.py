...
async def _ingest_document_in_background(
    document_id: int,
    file_name: str,
    data: bytes,
    *,
    ...
    """Run the expensive parse/embed/index pipeline after the HTTP response."""
    async with async_session() as db:
        try:
            ...
            # ingest_uploaded_file persists the failed status and error message.
...
async def upload_document(
    ...
    await _mark_document_processing(db, document)
    background_tasks.add_task(
        _ingest_document_in_background,
        document.id,
        file.filename,
        data,
    )
