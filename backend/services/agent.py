...
_current_document_ids: ContextVar[list[int] | None] = ContextVar(
...
def set_document_ids(document_ids: list[int] | None) -> Token:
    """Bind a document filter to the current async request context."""
    return _current_document_ids.set(document_ids)


def reset_document_ids(token: Token) -> None:
    """Restore the previous document filter after a streamed request."""
    _current_document_ids.reset(token)
...
async def knowledge_search(query: str) -> str:
    ...
    async with async_session() as db:
        results = _filter_evidence(await hybrid_search(
            ...
            query,
            top_k=5,
            document_ids=_current_document_ids.get(),
        ))
    return json.dumps(
