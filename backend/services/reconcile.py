...
RECONCILE_TIMEOUT_SECONDS = 150
RECONCILE_STALE_SECONDS = 5 * 60
...
async def get_running_task_id() -> str | None:
    ...
    task_id = await redis.get(_RUNNING_KEY)
    if task_id is None:
        return None
    ...
    if not started_at or time.time() - started_at > RECONCILE_STALE_SECONDS:
        await _finish_reconcile_task(
            task_id,
            status="failed",
            error="对账任务已超时，请检查 Milvus 状态后重试",
        )
        return None
    return task_id
...
async def run_reconcile_task(task_id: str) -> None:
    try:
        ...
        await _finish_reconcile_task(
            task_id,
            status="failed",
            error=f"对账超过 {RECONCILE_TIMEOUT_SECONDS} 秒，已终止等待；请检查 Milvus 集合加载状态",
        )
